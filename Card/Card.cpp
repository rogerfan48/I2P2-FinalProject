...
Card::Card(bool selected, int id, float x, float y, std::string Name, std::string Description, int pt, float radius, float duration, float interval, int atkTower): 
    Engine::IObject(x, y, CardWidth, CardHeight), cardType(SPELL), ID(id), Name(Name), Description(Description),
    pt(pt), radius(radius), duration(duration), interval(interval), atkTower(atkTower), selected(selected), inPlay(false) {
...
Card::Card(int id, float x, float y, std::string Name, int pt, float radius, float duration, float interval, int atkTower): 
    Engine::IObject(x, y, CardWidthP, CardHeightP), cardType(SPELL), ID(id), Name(Name),
    pt(pt), radius(radius), duration(duration), interval(interval), atkTower(atkTower), selected(false), inPlay(true) {
        bg = Engine::Resources::GetInstance().GetBitmap("card/SpellCard.png");
        head = Engine::Resources::GetInstance().GetBitmap("card/"+Name+".png");
        fontName = Engine::Resources::GetInstance().GetFont("recharge.otf", 40);
        fontDesc = Engine::Resources::GetInstance().GetFont("recharge.otf", 22);
