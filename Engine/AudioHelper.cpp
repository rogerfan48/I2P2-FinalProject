...
ALLEGRO_SAMPLE_ID AudioHelper::PlayAudio(const std::string& audio) {
	...
	if (!al_play_sample(sample, SFXVolume, 0.0, 1.0, ALLEGRO_PLAYMODE_ONCE, &id)) Engine::LOG(Engine::INFO) << "failed to play audio (once)";
	else Engine::LOG(Engine::VERBOSE) << "played audio (once)";
	return id;
}
...
ALLEGRO_SAMPLE_ID AudioHelper::PlayBGM(const std::string& audio) {
	ALLEGRO_SAMPLE* sample = Engine::Resources::GetInstance().GetSample(audio).get();
	ALLEGRO_SAMPLE_ID id;
	if (!al_play_sample(sample, BGMVolume, 0.0, 1.0, ALLEGRO_PLAYMODE_LOOP, &id)) Engine::LOG(Engine::INFO) << "failed to play audio (bgm)";
	else Engine::LOG(Engine::VERBOSE) << "played audio (bgm)";
	return id;
}
...
std::shared_ptr<ALLEGRO_SAMPLE_INSTANCE> AudioHelper::PlaySample(const std::string& audio, bool loop, float volume, float position) {
	...
	if (volume != 1) 	ChangeSampleVolume(smart_ptr, volume);
	if (position != 0) 	ChangeSamplePosition(smart_ptr, position);
    if (!al_play_sample_instance(sample_instance)) 	Engine::LOG(Engine::INFO) << "failed to play audio (sample)";
	else 											Engine::LOG(Engine::VERBOSE) << "played audio (sample)";
	return smart_ptr;
}
...
void AudioHelper::StopSample(std::shared_ptr<ALLEGRO_SAMPLE_INSTANCE> sample_instance) {
	if (!al_get_sample_instance_playing(sample_instance.get())) return;
	// No need to stop BGM by ourselves since the audio stops when the sample is destroyed.
	if (!al_stop_sample_instance(sample_instance.get())) Engine::LOG(Engine::INFO) << "failed to stop audio (sample)";
	else Engine::LOG(Engine::INFO) << "stopped audio (sample)";
}
